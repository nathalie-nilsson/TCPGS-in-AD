...
```{r}
score.files <- "analyses/adni/v2_prad_snps/1_pg_scoring/score_files/" # folder with score files
score.range <- "analyses/adni/v2_prad_snps/1_pg_scoring/scoring_range.txt" # scoring range file
```
...
## Data
```{r}
pheno <- read.within.project.h(pheno.file, header = TRUE) %>%
    rename(IID = iid)
head(pheno)
