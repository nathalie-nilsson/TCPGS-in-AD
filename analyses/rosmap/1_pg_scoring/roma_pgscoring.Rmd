---
title: "PG scoring of ROSMAP data based on Willer TC results"
...
## Notes
Instead of selecting SNPs by clumping (as was done in PREVENT-AD) the same SNPs that were used for scoring in PREVENT-AD are used for scoring in ROSMAP. This means that the score in both cohorts will be based on the exact same SNPs. 

## Input

#### Target data
```{r}
target.file <- "~/Research/Cohort_data/ROSMAP/genetic_data/imputed/rosmap_imp_eur" # .bed; .bim & .fam file
```

#### Base data
Paths relative to project base directory:
```{r}
base.file               <- "data/willer/willer_tc_rosmap_match/willer_tc_rosmap_match.txt" # summary file with header
base.file.flip.snps     <- "data/willer/willer_tc_rosmap_match/willer_tc_rosmap_match_SNPsFlip.txt" # file with SNPs to flip
```
