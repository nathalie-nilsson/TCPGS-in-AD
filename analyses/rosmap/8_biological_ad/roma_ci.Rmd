...
```{r}
atn <- pheno %>%
    filter(cerad.bin == 1 & !is.na(tc.pgs)) %>%
    mutate(sex = factor(sex.f, levels = c(0, 1), labels = c("Male", "Female"))) %>%
    select(iid, PC1:PC10, sex, apoe4.status, age, cog.ci, tc.pgs)
```
...
desc.stat <- sum.stat(atn.cc, strat_by = "sex", 
         ...
         disc_variables = c("apoe4.status", "cog.ci"))
...
```{r}
atn.formula <- as.formula("cog.ci~PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10+age+apoe4.status+sex*tc.pgs")
...
#### Odds ratios
```{r}
atn.formula.or <- as.formula("cog.ci~PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10+age+apoe4.status+tc.pgs")
