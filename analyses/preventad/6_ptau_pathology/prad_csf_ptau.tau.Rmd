---
title: "CSF p-tau/tau ratio at baseline ~ TC pgs in PRAD"
...
```{r}
csf <- mutate(pheno, csf.ptau.tau = csf.ptau/csf.tau) %>%
    filter(!is.na(csf.ptau.tau)) %>%
    select(iid, PC1:PC10, sex, apoe4.status, statin, tc.mM, csf.age, csf.ptau.tau, tc.pgs)
```
...
desc.stat <- sum.stat(m.dt, strat_by = "sex", 
         cont_variables = c("csf.age", "tc.mM", "tc.pgs", "csf.ptau.tau"), 
...
#### CSF p-tau/tau distribution
```{r}
grid.arrange(
    ggplot(csf.cc, aes(csf.ptau.tau)) + 
        geom_histogram(data = filter(csf.cc, sex == "Female"), bins = 10, fill = "#f5dd4dff") +
        geom_histogram(data = filter(csf.cc, sex == "Male"), bins = 10, fill = "#1f3c6dff") +
        my.theme, 
    
    ggplot(csf.cc, aes(csf.ptau.tau, fill = sex)) +
        geom_density(alpha = 0.5) +
        scale_fill_manual(values = c("#1f3c6dff", "#f5dd4dff")) +
        my.theme,
...
**Statistics**
```{r}
formula.1 <- as.formula("csf.ptau.tau ~ PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10+csf.age + apoe4.status + statin + sex * tc.pgs")
...
ggplot(csf.cc, aes_string(x = "tc.pgs", y = "csf.ptau.tau", color = "sex")) +
        geom_point(size = 3) +
...
## Save data
```{r}
save(list = c("csf", "desc.stat"), file = "prad_csf_ptau.tau.RData")
```
