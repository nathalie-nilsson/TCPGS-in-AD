...
```{r}
score.files <- "analyses/preventad/without_APOE/1_pg_scoring/score_files/" # folder with score files
score.range <- "analyses/preventad/without_APOE/1_pg_scoring/scoring_range.txt" # scoring range file
```

Phenotype data:
```{r}
pheno.file <- "prad_tc_boot_data.txt"

dep.var <- "tc.mM" # colname of dependent variable
covars <- c("sex", "age", "age2", "apoe4.status") # character vector with colnames of covariates
...
Output parameters:
```{r}
output.name.org <- "prad_tc_boot_statinsplit"
```
...
m.dt$sex <- abs(as.numeric(m.dt$sex) - 2) # 0 = F, 1 = M
